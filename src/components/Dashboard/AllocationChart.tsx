...
export function AllocationChart() {
    ...
    const { finalData, chartData } = useMemo(() => {
        ...
        const othersVal = sorted.slice(5).reduce((sum, item) => sum + item.currentValue, 0);

        const final = othersVal > 0
            ? [...main, { scrip: 'Others', currentValue: othersVal }]
            : main;

        return { finalData: final, chartData: sorted };
    }, [holdings]);
