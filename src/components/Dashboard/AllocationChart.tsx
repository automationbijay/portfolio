...
export function AllocationChart() {
    ...
    const { finalData, chartData } = useMemo(() => {
        // [Performance:Bolt] Memoize array spread, sorting, and slicing O(N log N)
        const data = [...holdings]
            .sort((a, b) => b.currentValue - a.currentValue);

        const mainAssets = data.slice(0, 5);
        const othersValue = data.slice(5).reduce((sum, item) => sum + item.currentValue, 0);

        const finalData = othersValue > 0
            ? [...mainAssets, { scrip: 'Others', currentValue: othersValue }]
            : mainAssets;

        return { finalData, chartData: data };
    }, [holdings]);
