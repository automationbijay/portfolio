...
export function AllocationChart() {
    ...
    // ⚡ Bolt: Wrapped chart data calculation in useMemo to prevent sorting and reducing on every render
    // Impact: Avoids O(N log N) sorting and O(N) reduction on every render cycle when dashboard updates
    const { finalData, chartData } = useMemo(() => {
        ...
        const processedData = othersValue > 0
            ? [...mainAssets, { scrip: 'Others', currentValue: othersValue }]
            : mainAssets;
        return { finalData: processedData, chartData: sortedData };
    }, [holdings]);
