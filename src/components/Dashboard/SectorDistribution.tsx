...
export function SectorDistribution() {
    ...
    const { sectorData, finalData } = useMemo(() => {
        ...
        const calculatedSectorData = Object.entries(sectorMap)
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value);

        const mainSectors = calculatedSectorData.slice(0, 5);
        const othersValue = calculatedSectorData.slice(5).reduce((sum, item) => sum + item.value, 0);

        const calculatedFinalData = othersValue > 0
            ? [...mainSectors, { name: 'Others', value: othersValue }]
            : mainSectors;

        return { sectorData: calculatedSectorData, finalData: calculatedFinalData };
    }, [holdings]);
