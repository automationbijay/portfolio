...
export function SectorDistribution() {
    ...
    // ⚡ Bolt: Wrap sector data calculations in useMemo to prevent mapping, reducing, and sorting on every render
    const { finalData, sectorData } = useMemo(() => {
        ...
        const processedSectorData = Object.entries(sectorMap)
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value);

        const mainSectors = processedSectorData.slice(0, 5);
        const othersValue = processedSectorData.slice(5).reduce((sum, item) => sum + item.value, 0);

        const calculatedFinalData = othersValue > 0
            ? [...mainSectors, { name: 'Others', value: othersValue }]
            : mainSectors;

        return { finalData: calculatedFinalData, sectorData: processedSectorData };
    }, [holdings]);
