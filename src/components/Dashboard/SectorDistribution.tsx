...
export function SectorDistribution() {
    ...
    // ⚡ Bolt Optimization: Memoize sector calculation to prevent unnecessary O(n log n) sorting on every re-render
    const { sectorData, finalData } = useMemo(() => {
        ...
        const sortedData = Object.entries(sectorMap)
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value);

        const mainSectors = sortedData.slice(0, 5);
        ...
        return { sectorData: sortedData, finalData: data };
    }, [holdings]);
