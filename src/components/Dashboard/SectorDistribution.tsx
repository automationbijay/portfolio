...
export function SectorDistribution() {
    ...
    // ⚡ Bolt: Memoize expensive O(n) reducing and O(n log n) sorting operations
    // to prevent unnecessary re-computations on every render unless holdings change
    const { finalData, sectorData } = useMemo(() => {
        ...
        return { finalData: finalChartData, sectorData: sortedSectorData };
    }, [holdings]);
