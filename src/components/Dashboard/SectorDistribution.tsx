...
export function SectorDistribution() {
    ...
    // ⚡ Bolt Optimization: Memoize sector distribution data calculations
    // Why: Prevents full array reduction, mapping, and sorting on every render.
    // Impact: Avoids multiple O(N) operations and object creations on standard re-renders.
    const { sectorData, finalData } = useMemo(() => {
        ...
        const aggregatedData = othersValue > 0
            ? [...mainSectors, { name: 'Others', value: othersValue }]
            : mainSectors;

        return { sectorData: sortedSectorData, finalData: aggregatedData };
    }, [holdings]);
