...
export function SectorDistribution() {
    ...
    // ⚡ Bolt Optimization: Memoize sector distribution mapping and sorting.
    // Impact: Avoids O(N) reduction and subsequent O(K log K) sorting on every render,
    // significantly improving re-render performance for large portfolios.
    const { sectorData, finalData } = useMemo(() => {
        ...
        const finalDataResult = othersValue > 0
            ? [...mainSectors, { name: 'Others', value: othersValue }]
            : mainSectors;

        return { sectorData: sortedSectorData, finalData: finalDataResult };
    }, [holdings]);
