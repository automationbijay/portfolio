...
export function SectorDistribution() {
    ...
    // ⚡ Bolt Optimization:
    // What: Added useMemo to cache expensive dictionary aggregation and array operations
    // Why: Prevents recalculating sector distribution on every render if holdings haven't changed
    // Impact: Avoids O(N) reduce, Object.entries, and sort operations during unneeded re-renders
    const { sectorData, finalData } = useMemo(() => {
        ...
        return { sectorData: sortedSectorData, finalData: calculatedFinalData };
    }, [holdings]);
