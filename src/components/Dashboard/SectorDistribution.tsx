...
export function SectorDistribution() {
    ...
    // ⚡ Bolt Optimization: Memoize sector distribution calculations to prevent O(N) reduce and O(K log K) sort on every render.
    // Expected impact: Faster re-renders and less CPU usage during dashboard interactions.
    const { finalData, sectorData } = useMemo(() => {
        ...
        const computedSectorData = Object.entries(sectorMap)
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value);

        const mainSectors = computedSectorData.slice(0, 5);
        const othersValue = computedSectorData.slice(5).reduce((sum, item) => sum + item.value, 0);

        const computedFinalData = othersValue > 0
            ? [...mainSectors, { name: 'Others', value: othersValue }]
            : mainSectors;

        return { finalData: computedFinalData, sectorData: computedSectorData };
    }, [holdings]);
