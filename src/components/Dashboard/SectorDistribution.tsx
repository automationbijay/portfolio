...
export function SectorDistribution() {
    ...
    const { sectorData, finalData } = useMemo(() => {
        // Performance: Memoize sector grouping, sorting, and aggregation to avoid expensive operations on every render
        const map = holdings.reduce((acc, item) => {
        ...
        const mainSectors = sorted.slice(0, 5);
        ...
        return { sectorData: sorted, finalData: data };
    }, [holdings]);
